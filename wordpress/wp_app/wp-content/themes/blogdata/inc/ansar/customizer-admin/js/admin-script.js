(function ($) {
  $(document).ready(function () {
    $('.blogdata-btn-get-started').on('click', function (e) {
      ...
      $.post(blogdata_ajax_object.ajax_url, {
        action: 'install_act_plugin',
        security: blogdata_ajax_object.install_nonce
      }, function (response) {
        if (response.success) {
          window.location.href = 'admin.php?page=ansar-demo-import';
        } else {
          alert((response.data && response.data.message) || 'Installation failed.');
  ...
  $(document).on('click', '.notice-get-started-class .notice-dismiss', function () {
    ...
    $.post(ajaxurl, {
      action: 'blogdata_dismissed_notice_handler',
      type: type,
    });
  });
})(jQuery);