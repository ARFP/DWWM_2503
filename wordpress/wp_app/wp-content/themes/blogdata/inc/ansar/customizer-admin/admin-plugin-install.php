            ...
            <div class="blogdata-notice clearfix">
                <div class="blogdata-notice-content">
                    ...
                    <div class="blogdata-hello">
                        ...
                        printf( esc_html__( 'Hello, %s', 'blogdata' ), esc_html( $current_user->display_name ) );
                        ?>
...
function blogdata_admin_info_install_plugin() {
    ...
    if ( ! current_user_can( 'install_plugins' ) ) {
        wp_send_json_error( array( 'message' => __( 'Sorry, you are not allowed to access this page.', 'blogdata' ) ), 403 );
    }

    // Security Nonce verification
    check_ajax_referer( 'blogdata_install_plugin_nonce', 'security' );
    ...
    if ( ! file_exists( WP_PLUGIN_DIR . '/' . $plugin_slug ) ) {
        ...
        if ( is_wp_error( $api ) ) {
            wp_send_json_error( array( 'message' => __( 'Failed to fetch plugin information.', 'blogdata' ) ) );
        }
        ...
        if ( is_wp_error( $result ) ) {
            wp_send_json_error( array( 'message' => __( 'Plugin installation failed.', 'blogdata' ) ) );
        }
    }
    if ( current_user_can( 'activate_plugins' ) ) {
        activate_plugin( 'ansar-import/ansar-import.php' );
    }
    wp_send_json_success( array( 'message' => __( 'Plugin installed and activated successfully.', 'blogdata' ) ) );
}